        ...
        public async Task<GetLogChartsOutput> GetCharts<T>(IRepository<T> repository) where T : class, ILogModel
        {
            ...
            for (var i = 0; i < days; i++)
            {
                ...
                else
                {
                    ...
                    output.All[i] = await repository.Count(x => x.LongDate >= dayTime && x.LongDate <= dayTime.AddHours(23).AddMinutes(59));
                    output.Error[i] = await repository.Count(x => x.LongDate >= dayTime && x.LongDate <= dayTime.AddHours(23).AddMinutes(59) && x.Level == "ERROR");
                }
            }
            return output;
        }
